...
const PortfolioModal: React.FC<PortfolioModalProps> = ({ professional, open, onOpenChange }) => {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-4xl w-full p-0 bg-background border-border text-foreground">
                <div className="grid grid-cols-1 md:grid-cols-3">
                    <div className="md:col-span-1 bg-secondary/50 p-8 flex flex-col items-center text-center">
                        <Avatar className="w-32 h-32 rounded-xl mb-4 border-4 border-background shadow-lg">
                            <AvatarImage src={professional.avatar} alt={professional.name} />
                            <AvatarFallback className="rounded-xl text-4xl bg-muted">{initials}</AvatarFallback>
                        </Avatar>
